...
class PlayState extends GameState
{
  ...
  public void test9( ) {
    // Ball Between Two Boosters
    resetObjects( );
    ball.setPosition( 1600, 725 );
    ball.setVelocity( 120, -40 );
  }

  public void load( ) {
    Vector2D center = new Vector2D( game.getWidth()*0.5, game.getHeight()*0.5 );
    bars = new FoosBar[8];
    bars[0] = new FoosBar( FIELD_LEFT+0.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 3, "tigers", false );
    bars[1] = new FoosBar( FIELD_LEFT+1.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 2, "tigers", false );
    bars[2] = new FoosBar( FIELD_LEFT+2.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 3, "dragons", true );
    bars[3] = new FoosBar( FIELD_LEFT+3.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 5, "tigers", false );
    bars[4] = new FoosBar( FIELD_LEFT+4.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 5, "dragons", true );
    bars[5] = new FoosBar( FIELD_LEFT+5.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 3, "tigers", false );
    bars[6] = new FoosBar( FIELD_LEFT+6.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 2, "dragons", true );
    bars[7] = new FoosBar( FIELD_LEFT+7.5*ZONE_WIDTH, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP), 3, "dragons", true );
    ...
    //vertWalls[4] = new Line( SCREEN_LEFT-400, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP-GOAL_SIZE), SCREEN_LEFT-400, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP+GOAL_SIZE) );
    //vertWalls[5] = new Line( SCREEN_RIGHT+400, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP-GOAL_SIZE), SCREEN_RIGHT+400, FIELD_TOP+0.5*(FIELD_BOTTOM-FIELD_TOP+GOAL_SIZE) );
    boosters = new Booster[14];
    ...
    boosters[12] = new BoosterGoal( SCREEN_LEFT+(FIELD_LEFT-SCREEN_LEFT)*0.5, (GOAL_TOP+GOAL_BOTTOM)*0.5, FIELD_LEFT-SCREEN_LEFT, GOAL_SIZE, new Vector2D( -1500, 0 ) );
    boosters[13] = new BoosterGoal( SCREEN_RIGHT-(SCREEN_RIGHT-FIELD_RIGHT)*0.5, (GOAL_TOP+GOAL_BOTTOM)*0.5, SCREEN_RIGHT-FIELD_RIGHT, GOAL_SIZE, new Vector2D( 1500, 0 ) );
    endLoad( );
  }
  ...
  public void draw( ) {
    ...
    if ( game.isDebugMode( ) ) {
      for ( int i = 0; i < touchables.length; i++ )
        ((TouchZone)touchables[i]).drawDebug( );
      for ( int i = 0; i < horzWalls.length; i++ )
        horzWalls[i].draw( );
      for ( int i = 0; i < vertWalls.length; i++ )
        vertWalls[i].draw( );
