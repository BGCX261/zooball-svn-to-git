...
abstract class GameState
{
  ...
  public void input( ) {
    if ( touchables != null && tacTile != null ) {
      int listSize = tacTile.dataListSize();
      for ( int i = 0; i < listSize; i++ ) {
        Touches touch = tacTile.getLeastRecentTouchData();
        Vector2D vector = new Vector2D( touch.getXPos( ) * game.getWidth( ), game.getHeight( ) - touch.getYPos( ) * game.getHeight( ) );
        
        if ( game.isDebugMode( ) ) {
          fill( 255, 0, 0 );
          ellipse( (float) vector.x, (float) vector.y, 30, 30 );
        }
      }
