public class FoosBar
{
  ...
  public FoosBar( double x, double y, int foosmen, String team, boolean top ) throws IllegalArgumentException {
    // Geometry
    FOOSMAN_WIDTH = 53;
  ...
  public void draw( ) {
    ...
    if ( top ) {
      y = 0;
      height = (float)(position[current].y + foosmenPositions[foosmenPositions.length - 1]);
      //rect( (float)position[current].x-6, 0, 14, (float)(position[current].y + foosmenPositions[foosmenPositions.length - 1]) );
    }
    else {
      y = (float)(position[current].y + foosmenPositions[0]);
      height = 1080 - y;
      //rect( (float)position[current].x-6, (float)(position[current].y + foosmenPositions[0]), 14, 1080 );
    }
    fill( 120 );
    rect( (float)position[current].x-7, y, 14, height );
    // FOOSMEN
    int r;
